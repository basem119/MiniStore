    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                await _userStore.SetUserNameAsync((Customer)user, Input.Email, CancellationToken.None);
                await _emailStore.SetEmailAsync((Customer)user, Input.Email, CancellationToken.None);

                var result = await _userManager.CreateAsync((Customer)user);
                if (result.Succeeded)
                {
                    result = await _userManager.AddLoginAsync((Customer)user, info);
                    if (result.Succeeded)
                    {
                        _logger.LogInformation("User created an account using {Name} provider.", info.LoginProvider);

                        var userId = await _userManager.GetUserIdAsync((Customer)user);
                        ...
                        await _signInManager.SignInAsync((Customer)user, isPersistent: false, info.LoginProvider);
                        return LocalRedirect(returnUrl);
                    }
                }
